...
export function AdminAgendaPanel({
  ...
  return (
    <section className="admin-panel" aria-labelledby="agenda-title">
      <div className="admin-panel-heading">
        <div>
          <p className="admin-kicker">Agenda</p>
          <h1 id="agenda-title">Horarios y bloqueos</h1>
          <p className="admin-help">
            Defini cuando trabaja cada profesional y bloquea horarios donde no se pueden tomar reservas.
          </p>
        </div>
      </div>
      ...
      <form className="admin-form" onSubmit={submitHours}>
        ...
        <button className="admin-primary-button">Guardar horarios</button>
      </form>

      <div className="admin-list compact">
        {hours.map((hour) => (
          <article className="admin-list-item" key={hour.id}>
            <div>
            ...
            <span className="admin-status active">
              {professionals.find((professional) => professional.id === hour.professionalId)?.name ??
                "General"}
            </span>
          </article>
        ))}
